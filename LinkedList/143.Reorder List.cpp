...
4.注意链表指针的移动除了指向还可以直接用到相等，比如反转过程中pre = cur;最后移动过程中head = next;
/**
...
12.12复习：
注意：
1.拆分：快慢指针需要判断快指针fast -> next && fast -> next ->next存在
2.反转：反转需要新建一个pre空结点，让头结点指向它，再向前移动
3.插入：插入过程中要记下该结点的下一位，方便另外一个链表反向指向
/**
 * Definition for singly-linked list.
 * struct ListNode {
 *     int val;
